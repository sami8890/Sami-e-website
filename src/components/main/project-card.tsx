...
export function ProjectCard({ project, onClick, index }: ProjectCardProps) {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -5 }}
      className="group relative rounded-xl overflow-hidden bg-zinc-900 cursor-pointer border border-zinc-800 shadow-lg hover:shadow-xl transition-all duration-300 h-full w-full focus-within:ring-2 focus-within:ring-[#00E188]"
      onClick={onClick}
      role="button"
      tabIndex={0}
      ...
      <div className="w-full h-[220px] sm:h-[240px] md:h-[260px] relative overflow-hidden bg-zinc-800">
        <div className="relative w-full h-full">
          ...
          <Image
            ...
            fill
            className={`object-cover object-center transition-all duration-700 ${imageLoaded ? "opacity-100" : "opacity-0"
          ...
          {/* Overlay on hover */}
          <div
            className={`absolute inset-0 bg-gradient-to-t from-black/70 to-transparent opacity-0 transition-opacity duration-300 ${isHovered ? "opacity-70" : ""
              }`}
          />
        </div>
      </div>
      ...
      <div className="relative p-5 sm:p-6 bg-zinc-900">
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <h3 className="text-lg sm:text-xl font-bold text-white line-clamp-2 transition-colors duration-300 group-hover:text-[#00E188]">
              {project.name}
            </h3>
          ...
          <div className="mt-4">
            <motion.button
              ...
              whileTap={{ scale: 0.97 }}
              className="w-full md:w-auto px-6 py-2.5 rounded-lg bg-[#00E188] text-black flex items-center justify-center gap-2 transition-colors hover:bg-[#00c077] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-zinc-900 focus:ring-[#00E188]"
              onClick={onClick}
              aria-label={`View details for ${project.name}`}
            >
              Overview
